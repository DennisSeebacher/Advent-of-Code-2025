...
class Perimeter:
    ...
    def FillThreaded(self, num_threads):
        print(f"Filling Holes (Threaded-{num_threads})(Prepare for Takeoff!) ({self.Height} rows per thread) ...")

        def fill_threadlet(r_start, r_end, r_id):
            print(f"#{r_id:8} -> copy data")
            
            height = r_end-r_start
            sub_perimeter = Perimeter(self.Width, height)
            ...
            for row in range(r_start, r_end):
                ...
                while col < self.Width:
                    if self.GetPoint(col, row) == 1:
                        sub_perimeter.SetPoint(col,row-r_start)
                        ...
                        for col2 in range(col + 1, self.Width):
                            if self.GetPoint(col2, row) == 1:
                                ...
                                #print(f"#{r_id:8} -> {row}/{col}-->{row}/{end}")
                                break
                        if end is not None:
                            for i in range(col + 1 , end):
                                sub_perimeter.SetPoint(i, row-r_start)
                            col = end
                            continue
                    col += 1

            print(f"#{r_id:8} -> finished")
            ranges.append((r_id, sub_perimeter))
