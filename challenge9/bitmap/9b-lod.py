...
# the bitmap is too big => scale it down => it's a bit inaccurate => do a rough pass, then do one or more finer passes
...
print("  Building scaled rectangles...")

scaled_rectangles = []
for rectangle in rectangles:
    scaled_rectangles.append(
                            Rectangle(
                                Coordinate(rectangle.coordinateA.x // perimeter_divisor, rectangle.coordinateA.y // perimeter_divisor),
                                Coordinate(rectangle.coordinateB.x // perimeter_divisor, rectangle.coordinateB.y // perimeter_divisor)
                            )
    )
...
for rectangle in scaled_rectangles:
    if counter > 0 and counter % 1000 == 0:
        print(f"{counter}/{len(scaled_rectangles)} => {(counter/len(scaled_rectangles))*100:.2f}%")
    counter += 1
    if scaled_perimeter.TestBorder(rectangle):
        valid_rectangle_ids.append(rectangle.id)
        print(f"+ {rectangle.id}")

scaled_rectangles = None
scaled_perimeter = None
scaled_coordinates = None

# ***** DETAIL PASS *****

# for each id in valid rectangle ids
# test at full resolution

# ***** SOLUTION *****

def rect_area_sort(r):
    return r.Area

print(f"Found {len(valid_rectangle_ids)} valid rectangle ids")

valid_rectangles = []
for rectangle in rectangles:
    if rectangle.id in valid_rectangle_ids:
        valid_rectangles.append(rectangle)

if len(valid_rectangles) > 0:
    valid_rectangles.sort(key=rect_area_sort, reverse=True)
    print(str(valid_rectangles[0]))

with open("9bout.txt", "w") as file:
    s = ""
    for item in valid_rectangle_ids:
        s += str(item)
        s += "\n"
    file.writelines(s)